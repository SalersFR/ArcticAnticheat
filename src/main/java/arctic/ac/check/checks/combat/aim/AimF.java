...
public class AimF extends Check {
    ...
    public AimF(PlayerData data) {
        super(data, "Aim", "F", "combat.aim.f", "Checks for great acceleration with smoothness in rotations (pitch).", true);
    }
    ...
    public void handle(Event e) {
        if (e instanceof RotationEvent) {
            ...
            if (!exempt && !exemptCombat) {
                if (accel <= 0.001F && (deltaYaw > 9.2D || (deltaYaw > 6.25 && deltaPitch > 0.125))) {
                    if (++buffer > 5) {
                        buffer /= 2;
