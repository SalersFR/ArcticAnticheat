...
public class ClientEntityLocations {
    PlayerData player;
    public HashMap<Integer, EntityId> getEntityFromId = new HashMap<>();

    public ClientEntityLocations(PlayerData p) {
        this.player = p;
    }
    ...
    public void relMove(int id, final double x, final double y, double z) {
        //TODO send Transtion
        sendTransaction(id, new Vector(getEntityFromId.get(id).getX() + (x / 32), getEntityFromId.get(id).getY() + (y / 32), getEntityFromId.get(id).getZ() + (z / 32)));
    }
